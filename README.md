# Lanstard Avatar 装扮切换器

VRChat 同一 avatar 多套衣服/配件装扮的批量上传 + OSC 切换工具集。
...
- 每个"装扮" = 一个 `AvatarVariantEntry` 配置项，包含：
  - `displayName`、`variantKey`（稳定 GUID）、`paramValue`、`thumbnail`、`menuIcon`、`uploadedName`、`uploadedDescription`
  - `includedRoots: List<GameObject>` —— 该装扮要保留的装扮根节点
- 批量上传时：当前装扮的 `includedRoots` 打 `Untagged`，其他所有装扮的受控 roots 打 `EditorOnly`，对同一个 avatar root 反复上传。每次得到一个新的 blueprint id，记录到 `variantKey → blueprintId` 映射文件。
...
| `Runtime/AvatarVariantEntry.cs` | `[Serializable]` 装扮条目 |
...
- 初版最多支持 **7 个装扮**（单层 Expression Menu 控件上限 8，SubMenu 入口占 1）。超过时 Validate 报错，分页能力下一版再加。
